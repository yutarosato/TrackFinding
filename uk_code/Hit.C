{
  ...
  double VN=48;
   ifstream ff("newocp_v48all.dat");
  //ifstream ff("newocp.dat");
  ...
  while(1){
    ...
    //phi digitize (vane)
    double Vdiv=360./VN;
    if((int)((P*180/TMath::Pi()+Vdiv/2.)/Vdiv)==VN) vane=0;
    else vane = (int)((P*180/TMath::Pi()+Vdiv/2.)/Vdiv);
  ...
  for(int ii=0; ii<NN;ii++){
    ...
    tt[ii]=ii*5;
    er[ii]=(double)TMath::Sqrt(newHit[ii])/(VN*2304*2)*100;
    newHit[ii]=(double)newHit[ii]/(VN*2304*2)*100;
    ex[ii]=2.5;
