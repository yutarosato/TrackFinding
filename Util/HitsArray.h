...
public:
  ...
  TH1D*    GetResidual_Hough_phiz( Int_t index ){ if( index>=m_hist_hough_phiz_resi.size() ){ std::cerr << "[ABORT] Wrong index for residual of hough-line(phiz)" << std::endl, abort(); } return m_hist_hough_phiz_resi.at(index); }
