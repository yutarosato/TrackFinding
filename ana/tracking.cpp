  ...
  for( Int_t ievt=0; ievt<nevt; ievt++ ){ // START EVENT-LOOP
    ...
    for( Int_t ihit=0; ihit<hits_info->GetNhits(); ihit++ ){
 	...
 	g_seed_hit_phiz->SetPoint( g_seed_hit_phiz->GetN(), hits_info->GetPhi(ivec), hits_info->GetZ(ivec) );
       ...
       TGraph* g_missing_hit_xy   = new TGraph(); g_missing_hit_xy  ->SetMarkerColor(1); g_missing_hit_xy  ->SetMarkerStyle(20);
       TGraph* g_missing_hit_phiz = new TGraph(); g_missing_hit_phiz->SetMarkerColor(1); g_missing_hit_phiz->SetMarkerStyle(20);
